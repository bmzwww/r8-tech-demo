...
def test_advanced_get_articles_time():
    ...
    assert totatl_time_in_seconds < 5 # less than 5 seconds

def test_advanced_get_articles_amount():
    result_one = src.optimized.get_articles()
    assert result_one.__len__() == 41 # amount of elements is 41

def test_advanced_load_page_negative():
    result_one = src.optimized.load_page(-1)
    assert result_one is None

def test_advanced_load_page_overflow():
    result_one = src.optimized.load_page(50)
...
def test_serialization():
    ...
    assert response['data'].__len__() == 10

def test_filter_articles():
    response = json.loads(example_page_text)    
    result_one = src.optimized.filter_articles(response['data'])
    assert result_one.__len__() == 9

def test_sort_articles():
    response = json.loads(example_page_text)    
    filtered_article = src.optimized.filter_articles(response['data'])
    result_one = src.optimized.sort_articles(filtered_article)
    assert result_one.__len__() == 9 and result_one[0]['title'] == 'F.C.C. Repeals Net Neutrality Rules'

def test_get_article_title():
    response = json.loads(example_page_text)    
    title = src.optimized.get_article_title(response['data'][0])
    title_2 = src.optimized.get_article_title(response['data'][8])
